...
Here's a [nice video](https://www.youtube.com/watch?v=dEXPMQXoiLc) walking through this.
...
- Review of MLP
  ...
  - After each linear layer comes an activation function, except for the last layer.
    ...
    - Instead, we use a variation of the categorical cross-entropy loss that can take the logits directly. (It can normalize them using the "logsumexp trick".) This is more numerically stable than softmax followed by computing the cross-entropy from the probabilities.
